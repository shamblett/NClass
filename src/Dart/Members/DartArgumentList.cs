...
{
    internal class DartArgumentList : ArgumentList
    {
        internal DartArgumentList()
        {
        }

        private DartArgumentList(int capacity) : base(capacity)
        {
        }
        ...
        {
            var param = DartParameterDeclaration.Create(declaration);

            if (IsReservedName(param.Name))
                throw new ReservedNameException(param.Name);

            var parameter = new DartParameter(param.Name, param.Type, param.Modifier, param.DefaultValue);
        ...
        public override Parameter ModifyParameter(Parameter parameter, string declaration)
        {
            ...
            var newParameter = new DartParameter(param.Name, param.Type, param.Modifier, param.DefaultValue);
        ...
        {
            DartArgumentList argumentList = new DartArgumentList(Capacity);
        ...
        {
            InitFromDeclaration(DartArgumentListDeclaration.Create(declaration));
        }
        ...
        private void InitFromDeclaration(IDartArgumentListDeclaration<IDartParameterDeclaration> declaration)
        {
            Clear();
            ...
            foreach (var param in declaration)
            {
                ...
                InnerList.Add(new DartParameter(param.Name, param.Type, param.Modifier, param.DefaultValue));
            }
        }
    }
}
